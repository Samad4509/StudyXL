...
class UniversityProgramController extends Controller
{
    ...
    public function store(Request $request, $university_id, $program_level_id, $field_of_studies_id, $intake_id)
{
    $intake = Intake::with('months')->findOrFail($intake_id); // fetch intake with related months
    $field_of_studies = FieldOfStudy::findOrFail($field_of_studies_id);
    $program_level = ProgramLevel::findOrFail($program_level_id);
    $university = University::findOrFail($university_id);

    // Validate request as you already have
    $validated = $request->validate([
        'program_name' => 'required|string|max:255',
        'program_description' => 'required|string',
        // ... other validations
        'students_requirements.*' => 'nullable',
    ]);

    $req = $request->students_requirements['english_exam_status'] ?? [];

    // Map intake months to JSON structure
    $intake_months = $intake->months->map(function ($month) {
        return [
            'id' => $month->id,
            'month' => $month->month,
            'open_date' => $month->open_date,
            'submission_deadline' => $month->submission_deadline,
            'status'    =>$month->status,
        ];
    });
    ...
    $program = UniversityProgram::create([
        ...
        // Store multiple intake months in JSON
